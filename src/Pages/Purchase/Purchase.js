...
const Purchase = () => {
    ...
    const [tools, setTools, qError] = useState({});
    const [user] = useAuthState(auth);
    let { minimumOrderQuantity } = tools;
    ...
    const handleSubmit = event => {
        ...
        if (minimumOrderQuantity > number) {
            return qError = <div className='text-red-500'> Can not order</div>
        }
        else {
            const url = `http://localhost:5000/tools/${purchaseId}`
            console.log(url)
            ...
            <div className='bg-white w-1/2 mx-auto rounded-lg drop-shadow-2xl p-2'>
                ...
                <div className='grid grid-cols-1 md:grid-cols-2 items-center bg-white'>
                    ...
                    <div className='font-sans'>
                        <h2 className='pb-1 text-2xl'>Name: {tools.name}</h2>
                        <hr />
                ...
                <div className='text-center pb-4'>
                    <div className="relative flex py-5 items-center">
                        ...
                        <span onChange={qError} className="flex-shrink mx-4 font-playfair text-2xl">Increase Quantity</span>
                        <div className="flex-grow border-t border-lime-500"></div>
                    </div>
